...
if grep -q " / .* ro," /proc/mounts 2>/dev/null; then
    IS_RO=true
    echo "Notice: Detected read-only root filesystem. Temporarily remounting rw..."
...
echo "[2/3] Sealing kernel loader via /etc/modprobe.d/blacklist-ebtables.conf..."
...
if lsmod | grep -q -E "^ebt"; then
    echo "Warning: Some ebtables modules remain resident in memory:"
    lsmod | grep -E "^ebt"
